...
def load_graph(v):
    G = nx.Graph()
    file_path = os.path.join(os.path.dirname(__file__), f'dti-{v}.csv')
    with open(file_path, encoding='utf-8') as f:
        reader = csv.DictReader(f)
        for row in reader:
            drug = row.get("DrugBank ID", "").strip()
            targets = row.get("Target Interactions", "").strip()
            if drug and targets:
...
def predict_top_targets(drug_id: str, top_n=5):
    ...
    for target_id in nodes:
        if not target_id.startswith('BE'):
            continue
        try:
